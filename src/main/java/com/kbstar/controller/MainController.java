...
//        model.addAttribute("weatherinfo", result); // jsp 파일에서 weatherinfo 로 사용
    ...
    public String loginimpl(Model model, String id, String pwd, HttpSession session) throws Exception { // 서버로 넘긴 정보 id, pwd, session
        logger.info(id +""+ pwd);
        Cust cust = null; // 받을 준비하기
        String nextPage = "loginfail";
        try {
            cust = service.get(id);
            //  get할 id 있고 && cust(DTO)가 가지고 있는 암호화된 pwd가 일치하면 = 로그인 성공
            if( cust != null && encoder.matches(pwd, cust.getPwd()) ){
                nextPage = "loginok"; // 페이지를 loginok로 교체하기.

                // 성공한 로그인 정보는 session에 logincust라는 이름으로 cust에 넣어주기.
                    // 기능 : session 에 넣어주면, 00초 동안 로그인 유지 가능.**
                    // session에 담은 정보도, jsp에서 logincust 라는 이름으로 정보 끄집어내기 가능하다.
